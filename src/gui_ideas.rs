...
impl Translate for Rect {
    fn translated(&self, pos: Vec2) -> Rect {
        Rect::new(self.x + pos.x, self.y + pos.y, self.w, self.h)
    }
}
...
pub trait GuiElement {
    ...
    fn update(&mut self, _mouse_pos: Vec2, _mouse_action: MouseAction, _parent_pos: Vec2) -> bool {
        false
    }
    // Sets the element to it's default state, should be called if update wasn't called
    fn default(&mut self) {}
    fn mouse_over(&self, _mouse_pos: Vec2, _parent_pos: Vec2) -> bool {
        false
    }
...
impl GuiElement for LabeledButton {
    ...
    fn update(&mut self, mouse_pos: Vec2, mouse_action: MouseAction, parent_pos: Vec2) -> bool {
        self.button
            .update(mouse_pos, mouse_action, self.pos + parent_pos)
    }
    fn default(&mut self) {
        self.button.default();
    }
