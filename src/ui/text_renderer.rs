...
impl TextRenderer {
    ...
    pub fn character_values(c: char) -> (f32, f32) {
        *CHAR_MAP.get(if CHAR_MAP.contains_key(&c) { &c } else { &ERROR_CHAR }).expect("ERROR_CHAR not a key in CHAR_MAP!")
    }
    ...
    pub fn draw_text(&self, text: &String, x: f32, y: f32, color: Color, line_gap: Option<f32>) {
        ...
        for c in text.chars() {
            ...
            draw_texture_ex(&self.chars_texture, x + x_pos, y + y_pos, color, DrawTextureParams {
    ...
    pub fn text_size(&self, text: &String, line_gap: Option<f32>) -> Vec2 {
        ...
        let largest_line_len: f32 = split_vector_by_indexes(&text.chars().collect::<Vec<char>>(), &line_breaks)
            .iter().map(|v| v.iter()
            .map(|c| TextRenderer::character_values(*c).1).sum())
            .fold(f32::NEG_INFINITY, f32::max);

        let height = (line_breaks.len()+1) as f32 * (self.chars_texture.height() + line_gap.unwrap_or(1.0)) - line_gap.unwrap_or(1.0);
