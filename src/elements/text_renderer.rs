...
pub fn default_char_map() -> HashMap<char, (f32, f32)> {
    HashMap::from([
        ...
        ('Z', (126.0, 5.0)), ('z', (232.0, 5.0)), ('¬', (  0.0, 1.0)), // used for aligning text by 1 pixel.. a bit hacky but meh
    ])                                                                 // (who tf is actually typing '¬'?!)
}

pub struct TextRenderer {
    // Map of where each char starts and ends in the texture, and it's real length in pixels
    map: HashMap<char, (f32, f32, f32)>,
    batch: InstanceArray,
...
impl TextRenderer {
    pub fn new_from_default(ctx: &mut Context) -> TextRenderer {
        let image = Image::from_bytes(ctx, DEFAULT_FONT_BYTES).expect("Error loading default image");
        TextRenderer::new(ctx, image, default_char_map())
    }
    ...
    pub fn text_size(&self, text: &String) -> Vec2 {
        ...
        Vec2::new(largest_line_len, self.height + (self.height + LINE_GAP) * (line_breaks.len()) as f32)
    }
