...
use super::{button::{self, LabeledButton}, TextRenderer};

pub enum DropdownItem {
    Button(LabeledButton), Divider(f32)
}
...
impl Dropdown {
    ...
    pub fn new(tr: &TextRenderer, item_names: Vec<Option<String>>, pos: Vec2, min_width: f32) -> Dropdown {
        let padding = (1.0, 1.0, 2.0, 2.0);
        // Make the width of the menu be the larger of the minimum width or the thickest button
        let width = min_width.max(item_names.iter().flatten().map(|s| tr.text_size_padded(&s, padding).x).fold(f32::NEG_INFINITY, f32::max));
