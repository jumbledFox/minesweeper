...
impl Button {
    ...
    pub fn update(&mut self, mouse_pos: Vec2, mouse_mode: MousePressMode) {
        ...
        self.state = match (mouse_mode, self.state, self.rect.contains(mouse_pos)) {
            ...
            (MousePressMode::Down, ..) => {
                if self.press_mode == PressMode::Immediate {
                    self.pressed = true;
                }
                State::Depressed
            }
            (MousePressMode::Up, ..) => {
                if self.press_mode == PressMode::Release {
                    self.pressed = true;
                }
                State::Hovered
            }
...
impl LabeledButton {
    pub fn new(
        ...
        LabeledButton {
            b: Button::new(
                Rect::new(pos.x, pos.y, dimensions.x, dimensions.y),
                press_mode,
                disabled,
            ),
            label,
    ...
    pub fn text_pos(&self) -> Vec2 {
        Vec2::new(
            self.b.rect.x + self.padding.2,
            self.b.rect.y + self.padding.0,
        )
    }
}
